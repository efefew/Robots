...
public class MusicManager : MonoBehaviour
{
    ...
    private void Start()
    {
        audioMixer.SetFloat("musicCutoffFrequency", minCutoffFrequency);
    }
    ...
    {
        StartCoroutine(ActivateCutoffFrequencyCoroutine(2, on));
        StartCoroutine(ChangeVolumeCoroutine(2, on));
    }
    /// <summary>
    ...
    private IEnumerator ActivateCutoffFrequencyCoroutine(float duration, bool on)
    {
        ...
        float elapsed = 0f;
        float startCutoffFrequency;
        audioMixer.GetFloat("musicCutoffFrequency", out startCutoffFrequency);
        float targetCutoffFrequency = on ? minCutoffFrequency : maxCutoffFrequency;

        //Выполняем код до тех пор пока не иссякнет время
        while (elapsed < duration)
        {
            audioMixer.SetFloat("musicCutoffFrequency", Mathf.Lerp(startCutoffFrequency, targetCutoffFrequency, elapsed / duration));
            elapsed += Time.deltaTime;
            yield return null;
        }
        audioMixer.SetFloat("musicCutoffFrequency", targetCutoffFrequency);
    }
    /// <summary>
