...
public class CameraOperator : MonoBehaviour
{
    ...
    public Limits limit;
    public ChangerSizeTiledImage tiledImage;

    private void OnEnable()
    {
        if (TargetTr != null)
            CamTr.position = new Vector3(TargetTr.position.x, TargetTr.position.y, 150);
    }
    ...
    private void NormalizePos(Transform trNorm, float height = 150) => trNorm.position = new Vector3(trNorm.position.x, trNorm.position.y, height);
    ...
    private void Zoom()
    {
        if (/*(AndroidInput.touchCountSecondary > 1 && AndroidInput.GetSecondaryTouch(1).radius < 0) || */Input.GetKey(KeyCode.E) && !(limit == Limits.point) || Input.GetAxis("Mouse ScrollWheel") >= 0.1)
        {
            ...
            if (tiledImage && MyCam.orthographicSize < whenChangeTiledImage)
                tiledImage.SetSizeTiledImage(true);
        }
        if (/*(AndroidInput.touchCountSecondary > 1 && AndroidInput.GetSecondaryTouch(1).radius > 0) || */Input.GetKey(KeyCode.Q) && !(limit == Limits.point) || Input.GetAxis("Mouse ScrollWheel") <= -0.1)
        {
            MyCam.orthographicSize += SpeedScale * (MyCam.orthographicSize / MaxZoom);
            ...
            if (tiledImage && MyCam.orthographicSize > whenChangeTiledImage)
                tiledImage.SetSizeTiledImage(false);
        }
    }
    ...
    public void ShakeCamera(float duration, float magnitude, float noize, Vector2 point, float maxForce = 3)
    {
        ...
        StartCoroutine(ShakeCameraCoroutine(duration, force * magnitude, force * noize));
    }
    /// <summary>
    ...
    private IEnumerator ShakeCameraCoroutine(float duration, float magnitude, float noize)
    {
        ...
        while (elapsed < duration)
        {
            ...
            CamTr.localPosition = CamTr.localPosition + (Vector3)cameraPostionDelta;
